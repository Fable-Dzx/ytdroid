...
- **ffmpeg**：用于合并音视频流 / 提取音频 / 嵌入字幕缩略图。官方不再发布安卓二进制，应用已内置**默认源一键安装**（Android arm64 预编译，含 ffmpeg / ffprobe 与核心共享库，托管在仓库 `ffmpeg-arm64` Release）：环境页点「安装」即自动下载安装，也可在「下载源」自定义 zip 直链或从本地 zip 安装；未安装时自动降级为单文件格式下载。
