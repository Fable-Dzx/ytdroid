...
- **ffmpeg**：用于合并音视频流 / 提取音频 / 嵌入字幕缩略图。官方不再发布安卓二进制，应用已内置**默认源一键安装**：默认从 bytedeco/JavaCPP 官方 Maven Central 下载 Android arm64 预编译包（含 ffmpeg / ffprobe 与核心共享库），自动镜像回退（阿里云 / Apache），仓库 Release 仅作最后兜底；也可在「下载源」自定义 zip/jar 直链或从本地文件安装；未安装时自动降级为单文件格式下载。
